...
def main():
    ...
    parser.add_argument("--language", type=str, default="en", choices=["en", "zh"],help="Language of the prompt")

    args = parser.parse_args()
    

    # load picabench
    geditbench = load_dataset("stepfun-ai/GEdit-Bench", cache_dir=args.data_path)
    ...
    gedit_en = [rec for rec in geditbench["train"] if rec["instruction_language"] == args.language]
    
    # Determine processing range
    total_samples = len(gedit_en)
    ...
    for idx in pbar:
        ...
        instruction_language = rec["instruction_language"]
        key_name = rec["key"]

        input_image = rec["input_image"]
        input_image = resize_image(input_image).convert("RGB")
        ...
        out_dir = out_root / "fullset" / f"{task_type}" / f"{instruction_language}"
        out_dir.mkdir(parents=True, exist_ok=True)
        out_path = out_dir / f"{key_name}.png"
